...
from app.models import User, Notification, Event, EventType
...
# --- 2. ЖИЗНЕННЫЙ ЦИКЛ (Без ручных миграций) ---
@asynccontextmanager
...
async def serve_sw(): 
    ...
    if not file_path.exists(): return Response(status_code=404)
...
async def delete_event(event_id: int, current_user_id: int = Depends(get_current_user), session: Session = Depends(get_session)):
    ...
    if not event or not user: return Response(status_code=404)
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    except Exception: manager.disconnect(websocket, user_id=user_id)
